...
ip=`ifconfig | grep -oE 'inet ([0-9]{1,3}\.?){4}\>' | grep -v "127\." | grep -oE '([0-9]{1,3}\.?){4}' | head -n 1`
Directory=/tmp/checklinux_7_fall5_$ip.log
if [ -f $Directory ];then
rm -f $Directory
fi
# ----------------
...
echo "<Code>systemctl list-unit-files | grep enabled | awk '{print \$1}' | grep -E '\.(sh|per|py)\$'</Code>" >> $Directory
echo "<Results>" >> $Directory
systemctl list-unit-files | grep enabled | awk '{print $1}' | grep -E '\.(sh|per|py)$' >> $Directory
echo "</Results>" >> $Directory
echo $divider >> $Directory
# ----------------
...
echo "<Code>ps -aux | sort -nr -k 3 | head -5 | awk '{if(\$3>=20) print \$0}'</Code>" >> $Directory
echo "<Results>" >> $Directory
ps -aux 2>/dev/zero | sort -nr -k 3 | head -5 | awk '{if($3>=20) print $0}' >> $Directory
echo "</Results>" >> $Directory
echo $divider >> $Directory
# ----------------
echo "<Name>47.View High MEM info</Name>" >> $Directory
echo "<Name>47.检查高内存占用</Name>" >> $Directory
echo "<Code>ps -aux | sort -nr -k 4 | head -5 | awk '{if(\$4>=20) print \$0}'</Code>" >> $Directory
echo "<Results>" >> $Directory
ps -aux 2>/dev/zero | sort -nr -k 4 | head -5 | awk '{if($4>=20) print $0}' >> $Directory
echo "</Results>" >> $Directory
echo $divider >> $Directory
# ----------------
